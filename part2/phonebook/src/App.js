...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    if (duplicatePerson) {
      if (window.confirm(`${duplicatePerson.name} is already added to the phonebook, replace the old number with a new one?`)) {
          phonebookService
          ...
          .then((returnedPerson) => {
            ...
            setNotification({
              type: 'success',
              message: `Changed ${newPerson.name}'s phone number`
            });
            setTimeout(() => {
